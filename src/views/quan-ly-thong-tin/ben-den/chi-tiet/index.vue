<template>
  ...
  >
    <p>
      Bạn có chắc chắn muốn xóa bến đến này không? Dữ liệu của bến đến này sẽ bị
      xóa vĩnh viễn. Hành động này không thể được hoàn tác.
    </p>
    <template #footer>
      <div class="flex items-center justify-end">
        <el-button @click="showDialogDelete = false">Hủy</el-button>
        <el-button type="danger" @click="handleXoaBenXe"> Xác nhận </el-button>
      </div>
    </template>
  </el-dialog>
</template>
...
export default {
  ...
  methods: {
    ...
    setTrangThai(value) {
      this.benDen.trangThai = value ? "Đang hoạt động" : "Ngừng hoạt động";
    },
    xacDinhChucNang() {
    ...
    async getChiTietBenDen(id) {
      try {
        ...
        if (result.status) {
          ...
          await this.danhSachStore.getDanhSachQuanHuyen(result.data.idTinh);
    ...
    async suaBenDen() {
      try {
        ...
        console.log(error);
        message("error", "Thay đổi thông tin bến đến thất bại, lỗi hệ thống! ");
      } finally {
        loading.stop();
      }
    },
    async handleXoaBenXe() {
      try {
        loading.start();
        const result = await this.benDenStore.DeleteByID(this.benDen.id);
        if (result.status) {
